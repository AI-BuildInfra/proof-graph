...
export class ResourceHandler {
  ...
  public readResource(uri: string): { contents: Array<{ uri: string; mimeType: string; text: string }> } {
    ...
    if (host === 'entities') {
      ...
      if (!entity) throw new Error(`Entity resource not found: ${uri}`);
    ...
    if (host === 'claims') {
      ...
      if (!claim) throw new Error(`Claim resource not found: ${uri}`);
    ...
    if (host === 'sources') {
      ...
      if (!source) throw new Error(`Source resource not found: ${uri}`);
      return {
        contents: [
          {
            uri,
