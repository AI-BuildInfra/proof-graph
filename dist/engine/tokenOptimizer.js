...
export const TRUST_CLASS_WEIGHTS = {
    standards_body: 1.0,
...
export class TokenOptimizer {
    static rankEvidence(evidenceList, sources, claimStatement) {
        return [...evidenceList].sort((a, b) => {
            ...
            const scoreA = trustA * 0.35 +
                relevanceA * 0.30 +
                (a.directness || 0.8) * 0.20 +
                (a.supports_claim ? 0.15 : 0.05);
            const scoreB = trustB * 0.35 +
                relevanceB * 0.30 +
    ...
    static deduplicateEvidence(evidenceList) {
        const clustered = [];
        const visited = new Set();
        for (let i = 0; i < evidenceList.length; i++) {
            if (visited.has(evidenceList[i].id))
                continue;
            const primary = { ...evidenceList[i] };
            const corroborating = [];
            for (let j = i + 1; j < evidenceList.length; j++) {
                if (visited.has(evidenceList[j].id))
                    continue;
    ...
    static buildPacket(options) {
        ...
        const ranked = this.rankEvidence(deduped, options.sources, options.claims[0]?.statement);
        ...
        packet.metrics.compression_ratio = Number((returnedTokens / Math.max(rawEstimatedTokens, returnedTokens)).toFixed(3));
        return packet;
    }
}
