...
export class AIDiscoverabilityEngine {
    ...
    evaluate(entityIdOrName) {
        ...
        const hasTech = relationships.some(r => ['DEVELOPS', 'MAINTAINS', 'PUBLISHES', 'HAS_REPOSITORY', 'HAS_PACKAGE'].includes(r.relationship.toUpperCase()));
        const techScore = hasTech ? 1.0 : 0.4;
        // 8. Third Party Corroboration
        const hasThirdParty = entitySources.some(s => ['established_publication', 'industry_directory', 'official_registry', 'academic_source'].includes(s.trust_class));
        ...
        if (idClarityScore < 0.8)
            recommendations.push('Provide a rich, factual description of core entity capabilities and origin.');
        if (sourceScore < 0.8)
            recommendations.push('Attach additional authoritative source URLs across varied domains.');
        if (relScore < 0.8)
            recommendations.push('Map explicit relational edges (e.g. DEVELOPS, OWNS, PUBLISHES) to associated projects.');
        if (thirdPartyScore < 0.8)
            recommendations.push('Include third-party registry listings or independent publication citations.');
        if (evScore < 0.8)
            recommendations.push('Extract specific supporting text excerpts verifying core claims.');
        return {
            entity: entity.name,
            entity_id: entity.id,
...
//# sourceMappingURL=aiDiscoverability.js.map