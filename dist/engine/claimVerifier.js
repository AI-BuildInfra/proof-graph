...
export class ClaimVerifier {
    ...
    verify(claimStatement, entityId) {
        ...
        for (const c of allClaims) {
            if (entityId && c.subject_id !== entityId && c.object_id !== entityId)
                continue;
        ...
        for (let i = 0; i < results.length; i++) {
            for (let j = i + 1; j < results.length; j++) {
                if (results[i].status !== results[j].status && results[i].status !== 'unverified' && results[j].status !== 'unverified') {
                    discrepancies.push(`Divergence detected: "${results[i].claim}" is [${results[i].status}] whereas "${results[j].claim}" is [${results[j].status}].`);
                }
            }
        }
...
//# sourceMappingURL=claimVerifier.js.map